...
import { CheckCircleOutlined, FileDoneOutlined, ArrowLeftOutlined } from "@ant-design/icons";
...
export default function RenterCheckoutAssetPage() {
  ...
  const handleConfirm = async () => {
    ...
    // Lấy các asset chưa kiểm kê trả phòng
    ...
    const allAssetsWithoutStatus = assets.filter(a => {
      const id = a.assetId || a.id;
      const selected = actualStatusState[id] || a.assetStatus || a.condition || a.status;
      return !selected || String(selected).trim() === "";
    });
    ...
    try {
      ...
      message.success("Đã lưu kiểm kê tài sản trả phòng thành công!");
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        return (
          <Input.TextArea
            placeholder="Ghi chú tình trạng khi trả phòng..."
            value={notes[id] || ""}
