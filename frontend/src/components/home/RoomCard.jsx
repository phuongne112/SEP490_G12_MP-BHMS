...
import { CheckCircleOutlined, ExclamationCircleOutlined, RestOutlined, ThunderboltOutlined } from '@ant-design/icons';
...
const getStatusTag = (status) => {
    switch (status) {
        ...
        case 'Occupied':
            return <Tag icon={<RestOutlined />} color="error">Occupied</Tag>;
        case 'Maintenance':
            return <Tag icon={<ThunderboltOutlined />} color="warning">Maintenance</Tag>;
        default:
    ...
    return (
        ...
        >
            ...
            <div style={{ padding: '20px' }}>
                <Title level={4} style={{ marginBottom: 4, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{room.roomNumber}</Title>
                
                <Text type="secondary" style={{ display: 'block', marginBottom: 12 }}>{room.area} m²</Text>

                <Title level={5} style={{ color: '#1890ff' }}>
                    {room.pricePerMonth?.toLocaleString('en-US')} <Text style={{ fontSize: 14, color: '#1890ff' }}>VND/month</Text>
                </Title>
                
                <Space direction="vertical" style={{ width: '100%', marginTop: 16 }}>
                    <Space size="large">
                        <Text type="secondary">🛏️ {room.numberOfBedrooms} Bedrooms</Text>
                        <Text type="secondary">🛁 {room.numberOfBathrooms} Bathrooms</Text>
                    </Space>
                     <Text type="secondary">🛋️ Furnished: <Text strong style={{ color: hasAssets ? '#52c41a' : '#bfbfbf' }}>{hasAssets ? 'Yes' : 'No'}</Text></Text>
                </Space>
            </div>
        </Card>
    );
